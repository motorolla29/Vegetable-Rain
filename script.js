...
window.addEventListener('DOMContentLoaded', () => {
  ...
  Raindrop.prototype.isOffscreen = function () {
    return (
      this.y > screenSize.HEIGHT + this.size * 5 || this.x < -this.size * 5
    );
  };
  ...
  var setup = function () {
    ...
    var raindrops = new Array(DROPS * CUCUMBER_RATIO)
      ...
      .concat(
        new Array(DROPS * TOMATO_RATIO).fill('').map(function () {
          return new Tomato();
        })
      )
      .concat(
        new Array(DROPS * POTATO_RATIO).fill('').map(function () {
          return new Potato();
        })
      )
      .concat(
        new Array(DROPS * CARROT_RATIO).fill('').map(function () {
          return new Carrot();
        })
      );
